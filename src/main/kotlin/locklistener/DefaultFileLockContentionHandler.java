...
public class DefaultFileLockContentionHandler implements FileLockContentionHandler, Stoppable {
                ...
                while (true) {
                    try {
                        ...
                        if (received.isPresent()) {
                            ...
                            lock.lock();
                            try {
                                ContendedAction contendedAction = contendedActions.get(payload.lockId);
                                if (contendedAction == null) {
                                    ...
                                    communicator.confirmUnlockRequest(packet.getSocketAddress(), payload.lockId);
                                }
    ...
    private void acceptConfirmationAsLockRequester(FileLockPacketPayload payload, Integer port) {
        long lockId = payload.lockId;
        if (payload.type == LOCK_RELEASE_CONFIRMATION) {
